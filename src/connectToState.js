...
export default function connectToState(CreditCardInput) {
  class StateConnection extends Component {
    ...
    setValues = values => {
      ...
      if (this.initialNumber === null && !requiresNumber) {
        this.initialNumber = values.number;
      }
      if (this.initialNumber !== null) {
        newValues.number = this.initialNumber;
      }
      ...
      const validation = (new CCFieldValidator(displayedFields, this.props.validatePostalCode, requiresNumber)).validateValues(formattedValues);
